<template>
    <div class="flex flex-col md:flex-row space-x-0 md:space-x-12 space-y-6 md:space-y-0">
        <img :src="post.image.url" :alt="post.title" class="w-full md:w-1/2 h-48 md:h-96 shadow rounded object-center object-center object-cover">
        ...
        <div class="w-full md:w-1/2 grid grid-cols-1 content-between space-y-4">
            ...
            <p class="w-full" v-if="post.content.length >= 400">{{ post.content.substring(0,400) + "..." }}</p>
